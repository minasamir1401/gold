...
export function CurrencyTable() {
    ...
    return (
        <div className="glass-card overflow-hidden border-gold-500/20 shadow-2xl shadow-gold-500/5 transition-all duration-500">
            <div className="premium-header flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest">{t.currencies.update_instant}</p>
                    </div>
                </div>
            </div>
                ...
                <table className={`w-full ${isRTL ? "text-right" : "text-left"} border-collapse`}>
                    <thead>
                        <tr className="bg-slate-50/50 dark:bg-slate-900/30">
                            ...
                            <th className="px-6 py-4 text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">{t.currencies.table_code}</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {currencies.map((curr, i) => (
                            ...
                            >
                                ...
                                <td className="px-6 py-5 font-bold text-slate-400">{curr.code}</td>
                                <td className="px-6 py-5 text-lg font-black text-emerald-600 dark:text-emerald-400">{curr.buy}</td>
            ...
            <div className="premium-footer flex items-center justify-center gap-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                <div className="flex items-center gap-1">
                    <Clock className="h-3 w-3" />
                    <span>{t.currencies.auto_update}</span>
                </div>
                <span>•</span>
                <span>{t.currencies.source}: Sarf Today</span>
            </div>
        </div>
    );
}
